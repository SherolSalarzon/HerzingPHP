...
function getIdExist($connection, $username, $email)
{
    ...
    if (mysqli_num_rows($sub) > 0) {
        ...
        if ($email == isset($row['email']) || $username == isset($row['username'])) {
            $info = array($row['ID'], $row['username'], $row['password'], $row['email']);
            return true;
        } else {
            return false;
        }
...
function createUsers($connection, $username, $firstname, $lastname, $password, $province, $email)
{
    ...
    // Slashes the entry before it enters the database.
    addslashes($username);
...
function passwordVerification($name, $password)
{
    $connect = getConnection();
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        if (
            $name == isset($row['email']) || $name == isset($row['username'])
...
function loginUser($connection, $userOrEmail, $password)
{
    ...
    if ($passExist && $userExist) {
        ...
        header("location:../loginPage.php?error:wrong-info?userExist=" . $userExist . "?passExist=" . $passExist);
        exit;
    }
