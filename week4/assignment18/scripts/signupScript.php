...
if (isset($confirm) && $_POST['g-recaptcha-response'] != "") {
	// Making sure the there it ignores html tags on the input tags
	$key = "6Le7LqYjAAAAABfny58OkMJ7zx685ubrKIQeam7m";
	$verify = file_get_contents('https://www.google.com/recaptcha/api/siteverify?secret=' . $key . '&response=' . $_POST['g-recaptcha-response']);
	...
	if ($hasError) {
		...
		if ($response->success) {
			if (getIdExist($connection, $username, $email)) {
				// If exist
				header("location:../loginPage?error:account=true");
				exit();
			} else {
			...
			header("location:../registerPage?error=");
		}
